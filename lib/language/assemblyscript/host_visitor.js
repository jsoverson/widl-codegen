...
class HostVisitor extends widl_1.BaseVisitor {
    ...
    visitOperation(context) {
        ...
        operation.arguments.map((arg, index) => {
            ...
            this.write(`${arg.name.value}: ${_1.expandType(arg.type, true, false)}`);
        });
        this.write(`): ${_1.expandType(operation.type, true, false)} {\n`);
        this.write(`  `);
      ...
      inputArgs.toBuffer()
        ...
        if (!retVoid) {
            ...
            if (_1.isObject(operation.type)) {
                ...
                if (operation.type instanceof widl_1.Optional) {
                    this.write(`    let ret: ${_1.expandType(operation.type, true, false)} = null;
            if (!decoder.isNextNil()) {
              ${_1.read("ret", operation.type.type, false, _1.isReference(operation.annotations))}
            }\n`);
                }
                else {
