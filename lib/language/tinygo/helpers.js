 ...
 * @param isReference if the type that is being expanded has a `@ref` annotation
 */
function read(variable, errorHandling, defaultVal, t, prevOptional, isReference) {
    ...
    switch (true) {
      ...
      if err != nil {
        ...
        mapSize--\n`;
            mapCode += read("key", true, defaultVal, t.keyType, false, isReference);
            if (!mapCode.endsWith(`\n`)) {
                mapCode += `\n`;
            }
            mapCode += `if err != nil {
          return ${returnPrefix}err
            ...
            mapCode += read("value", true, defaultVal, t.valueType, false, isReference);
            if (!mapCode.endsWith(`\n`)) {
                mapCode += `\n`;
            }
            mapCode += `if err != nil {
      ...
      if err != nil {
        ...
        var nonNilItem ${t.type instanceof widl_1.Optional ? "*" : ""}${exports.expandType(t.type, undefined, false, isReference)}\n`;
            listCode += read("nonNilItem", true, defaultVal, t.type, false, isReference);
        ...
        case t instanceof widl_1.Optional:
            ...
            optCode += read(variable, false, defaultVal, optNode.type, true, isReference);
            optCode += "}\n";
            optCode += "}\n";
            return optCode;
        default:
...
function write(typeInst, typeClass, typeMeth, variable, t, prevOptional, isReference) {
    ...
    switch (true) {
      ...
      for k, v := range ${variable} {
        ${write(typeInst, typeClass, typeMeth, "k", mappedNode.keyType, false, isReference)}${write(typeInst, typeClass, typeMeth, "v", mappedNode.valueType, false, isReference)}}
      }\n`;
            return code;
        case t instanceof widl_1.List:
            const listNode = t;
      ...
      for _, v := range ${variable} {
        ${write(typeInst, typeClass, typeMeth, "v", listNode.type, false, isReference)}}\n`;
            return code;
        case t instanceof widl_1.Optional:
            ...
            code += write(typeInst, typeClass, typeMeth, vprefix + variable, optionalNode.type, true, isReference);
            code += "}\n";
            return code;
        default:
            return "unknown";
